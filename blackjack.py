...
Original file is located at
    https://colab.research.google.com/github/rym621jp/BlackJack/blob/main/BlackJack.ipynb
"""
...
class Players():   #人数, 初期所持チップ
    class Player():
        ...
        def lose(self):
            self.betting = 0
            if(self.chips==0):
                print("{}さん->所持チップが0になりました。ゲームオーバーです。".format(self.name))
            ...
            if(hide==0):
                if(self.bust):
                    if(self.isDealer):
                        ...
                        print("{}さん->バーストしました。".format(self.name))
...
#認められる値が入力されるまで繰り返しinput()し続ける
def modInput(text,func=int,option=lambda hoge :True):   #input()のオプション, 目的の型, 入力値を判定してboolで返す関数
    while True:
        try:
...
player = modInput("プレイ人数を入力してください")
initialChips = modInput("初期所持チップ数を入力してください(最小単位10)",int,lambda num: num%10==0 and num>0)
